...
export class CoordinatorEditCompany extends React.Component
{
  constructor(props){
    super(props);
          ...
          axios.get('/backend/coordinator/companies/'+this.state.id,{
            ...
            .then((response) => {
              ...
              this.setState({
                ...
                designation1:data.contact1.designation,
                mailId1:data.contact1.mailId,
                mobileNumber1:data.contact1.mobileNumber,
              })
              if(data.contact2)
              {
                this.setState({
                  name2:data.contact2.name,
                  designation2:data.contact2.designation,
                  mailId2:data.contact2.mailId,
                  mobileNumber2:data.contact2.mobileNumber,
                });
              }
              if(data.contact3)
              {
                this.setState({
                name3:data.contact3.name,
                designation3:data.contact3.designation,
                mailId3:data.contact3.mailId,
                mobileNumber3:data.contact3.mobileNumber,
              });
              }
            })
            .catch((e)=>{
      ...
      <div className="base-container p-1">
        <section className="container-fluid border p-1 rounded border-success admin m-3">
          <form className="form" onSubmit = {this.handleSubmit} onChange={this.handleChange}>
            ...
            <button type="submit" className="btn btn-primary btn-block">Edit Company</button>
          </form>
        </section>
      </div>
  );
